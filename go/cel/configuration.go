...
import (
	...
	_ "chromium.googlesource.com/enterprise/cel/go/common"
	_ "chromium.googlesource.com/enterprise/cel/go/gcp/cloudkms"
	_ "chromium.googlesource.com/enterprise/cel/go/gcp/compute"
	_ "chromium.googlesource.com/enterprise/cel/go/host"
)
...
type Configuration struct {
	...
	// at deployment time.
	Resources hostpb.RuntimeSupport `json:"-"`

	// Startup dependencies. This section should not be specified in any
	// intput. Instead it is generated by the deployer and graftted onto the
	// namespace at deployment time.
	Startup hostpb.Startup `json:"-"`

	// Tracks the location of the completed asset manifest once it's been
	// generated and uploaded.
	Manifest commonpb.FileReference `json:"-"`
	...
	assetSources map[string]*assetpb.AssetManifest

	// Paths of source files that were merged into HostEnvironment. The mapping
	// is from the absolute path to the data that was loaded from it.
	hostSources map[string]*hostpb.HostEnvironment
...
func (c *Configuration) mergeAssets(filename string, data []byte) error {
	if c.assetSources == nil {
		c.assetSources = make(map[string]*assetpb.AssetManifest)
	}
...
func (c *Configuration) mergeHost(filename string, data []byte) error {
	if c.hostSources == nil {
		c.hostSources = make(map[string]*hostpb.HostEnvironment)
	}
...
func (c *Configuration) Validate() error {
	// Validate() is idempotent and should be able to be called multiple times.
