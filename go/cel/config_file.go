...
// host environment.
func (l *LabConfiguration) Merge(filename string) error {
	if l.sealed {
		return NewConfigurationError(filename, false, errors.Errorf("configuration is already sealed"))
	}
...
func (l *LabConfiguration) Validate() error {
	...
	err_list := []error{
		l.References.Collect(&l.AssetManifest, AssetRootPath),
		l.References.Collect(&l.HostEnvironment, HostRootPath),
		l.References.ResolveWith(&l.AssetManifest, AssetRootPath, common.ResolutionSkipOutputs),
		l.References.ResolveWith(&l.HostEnvironment, HostRootPath, common.ResolutionSkipOutputs),
		l.References.ResolveInlineRefs(&l.AssetManifest, AssetRootPath),
		l.References.ResolveInlineRefs(&l.HostEnvironment, HostRootPath),
...
func unmarshallTextProtoFromFile(filename string, p common.RefPath, m proto.Message) (err error) {
	defer Action(&err, "reading text format protobuf file \"%s\"", filename)
